...
{   
    ...
    if (action==GLFW_PRESS){
        if(key == GLFW_KEY_R)glPolygonMode( GL_FRONT_AND_BACK, GL_LINE);
...
int main(void)
{
    ...
    glm::vec3 direction = glm::vec3(cos(glm::radians(yaw))*cos(glm::radians(pitch)),sin(glm::radians(pitch)),(-1)*sin(glm::radians(yaw))*cos(glm::radians(pitch)));
    ...
    {   
        ...
        if(cameraPos[1]<1)
            cameraPos[1]=1;
        direction = glm::vec3(cos(glm::radians(yaw))*cos(glm::radians(pitch)),sin(glm::radians(pitch)),(-1)*sin(glm::radians(yaw))*cos(glm::radians(pitch)));
        cameraFront=glm::normalize(direction);
        view = glm::lookAt(cameraPos,cameraPos+cameraFront,cameraUp);
        glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(view));
        for (float i = 0; i < 10; i+=1)
        {
            for (float j = 0.0f; j < 10; j+=1)
            {
                for (float k = 0.0f; k < 10; k+=1)
                {
                    model = glm::mat4(1.0f);
                    model = glm::translate(model, glm::vec3((i*4)-18,k*4,j*-4));
                    glUniformMatrix4fv(modelLoc,  1, GL_FALSE, glm::value_ptr(model));
                    glUniform4f(glGetUniformLocation(shaderProgram,"color"),i/10,j/10,k/10,1.0);
        ...
        for (float i = 0; i < 101; i++)
        {
            ...
            {
                model = glm::mat4(1.0f);
                model = glm::translate(model, glm::vec3(i-50,-1,(j*-1)+32));
                glUniformMatrix4fv(modelLoc,  1, GL_FALSE, glm::value_ptr(model));
